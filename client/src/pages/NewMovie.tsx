...
export function NewMoviePage() {
  ...
  const { trigger: createMovie, loading, error } = useAsyncAction(async (event: FormEvent) => {
    ...
    await movieService.create({
      ...
      country_of_origin:input.country_of_origin ? input.country_of_origin : "",
      release_date:new Date(input.releaseDate) ? new Date(input.releaseDate): new Date(),
      budget:Number(input.budget) ? Number(input.budget) : 0,
